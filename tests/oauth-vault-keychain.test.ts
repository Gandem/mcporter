...
describe('oauth vault keychain backend', () => {
  ...
  it('forces generated CLIs to the file vault without changing their inherited environment', () => {
    const env = {
      MCPORTER_CREDENTIAL_BACKEND: 'keychain',
      MCPORTER_OAUTH_VAULT_BACKEND: 'keychain',
      MCPORTER_VAULT_BACKEND: 'keychain',
    };
    const originalAliases = { ...env };
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});

    forceFileVaultBackend(env);

    expect(env).toEqual(originalAliases);
    expect(resolveOAuthVaultBackend(env)).toBe('file');
